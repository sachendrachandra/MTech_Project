...
def main(argv):
  ...
  for lines in code:
    ...
    li=[]
    li2=[]
    print(len(subtokenized_sentences))
    try:
  ...
  with open(FLAGS.output_filepath,'a') as output_file:
    for i in range(len(li_final)):
      print(len(li_final[i]))
      for j in range(len(li_final[i])):
        output_file.write(str(li_final[i][j]))
        if(j!=len(li_final[i])-1):
          output_file.write(",")
      output_file.write("\n")
